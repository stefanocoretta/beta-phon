...
\bibliographystyle{elsarticle-harv} %DIF > 
 %DIF > 
 %DIF > 
%DIF -------
\makeatletter
...
%DIF < \bibliographystyle{elsarticle-harv}
%DIF -------
\usepackage{bookmark}
%DIF 183d194
%DIF < 
...
\hypersetup{
  ...
  pdfkeywords={acoustics, articulation, proportions, Bayesian
regressions, beta distribution},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
English Language},addressline={3 George Sq},city={Edinburgh, United
Kingdom},postcode={EH8 9AD},postcodesep={}}
\affiliation[2]{organization={TU Dortmund University, Department of
Statistics},city={Dortmund, Germany},postcode={44221},postcodesep={}}
...
Another commonly employed type of outcome variable \DIFdelbegin \DIFdel{are proportions}\DIFdelend \DIFaddbegin \DIFadd{is data that is
bounded between 0 and 1}\DIFaddend : for example, proportion of voicing during
closure \citep{davidson2016}, vocal folds contact quotient
\citep{herbst2017}, gesture amplitude \citep{carignan2021}, nasalance
\citep{carignan2021}\DIFdelbegin \DIFdel{. Moreover, virtually any measure can be MIN-MAX normalised, a procedure which
transforms values so that they are in the range 0--1}\DIFdelend \DIFaddbegin \DIFadd{, and data from slider answers where participants
can indicate responses for qualities}\DIFaddend .
...
\DIFaddbegin \DIFadd{Furthermore, a Gaussian distribution predicts values above 1, which are
not possible given the bounded nature of the data. }\DIFaddend Moreover, the density
peak (median) of the Gaussian distributions is lower than the peak in
the original data.
...
distribution for this purpose is the beta distribution. \DIFaddbegin \DIFadd{Note that the
beta distribution supports variables in the interval }{[}\DIFadd{0, 1}{]}\DIFadd{,
i.e.~excluding 0 and 1; variables in the interval (0, 1), including 0
and 1, can be modelled with zero and zero/one inflated beta
distributions (see Section~\ref{sec-beta}). }\DIFaddend This tutorial introduces
...
assumed, but no prior knowledge of Bayesian statistics is necessary. The
rest of the paper is structured as follows: Section~\ref{sec-beta}
introduces the mathematical underpinnings of the beta distribution,
Section~\ref{sec-bayes} provides the readers with a brief explanation
and justification of Bayesian regression models, while
Section~\ref{sec-case-1} and Section~\ref{sec-case-2} illustrate how to
fit Bayesian beta regression models with two case studies. The research
compendium with data and code can be found at
\url{https://github.com/stefanocoretta/beta-phon}.

\section{The beta distribution}\label{sec-beta}

The beta distribution is often considered as \emph{the} distribution for
the modelling of unit interval data\DIFaddbegin \DIFadd{, i.e.~data that are in the interval
}{[}\DIFadd{0, 1}{]} \DIFaddend \citep{ferrari_beta_2004, cribari-neto_beta_2010}. It has
...
variance
\(\text{Var}(y) = \frac{\mu \, (1 - \mu)}{\phi + 1}\).\DIFaddbegin \footnote{\DIFadd{The
  \(\mu\), \(\phi\) parametrisation is equivalent to the \(\alpha\),
  \(\beta\) parametrisation in that \(\alpha = \mu\phi\) and
  \(\beta = (1-\mu)\phi\). Using the former allows brms to exploit the
  same programmatic features of other distributions parametrised with a
  location parameter.}} \DIFaddend That is, the larger \(\phi\) the smaller the
variance of the corresponding beta distribution. While the mathematical
details are not needed to understand the content of this tutorial, we
still write down the density below for reasons of completeness:
...
\DIFaddbegin \DIFadd{See Figure~\ref{fig-beta} for an illustration of different beta
distributions based on various \(\mu\) and \(\phi\) values.
}\DIFaddend 
...
\pandocbounded{\includegraphics[keepaspectratio]{manuscript_files/figure-pdf/fig-beta-1.pdf}}

}

\caption{\label{fig-beta}\DIFaddFL{Beta distributions for various \(\mu\),
\(\phi\) values.}}

\end{figure}%DIF > 

\DIFadd{As mentioned earlier, the beta distribution handles data in the interval
}{[}\DIFadd{0, 1}{]}\DIFadd{, thus excluding 0 and 1. To model (0, 1) data (including 0
and 1), the zero and zero/one inflated extensions of the beta
distribution can be used. Both of these are supported in the software
used in this tutorial, but will not be treated here. We refer readers to
\mbox{%DIFAUXCMD
\citet{heiss2021}}\hskip0pt%DIFAUXCMD
.
}

\DIFaddend \section{Bayesian regression models}\label{sec-bayes}
...
\DIFdelbegin \DIFdel{most }\DIFdelend \DIFaddbegin \DIFadd{some }\DIFaddend areas of phonetics \DIFdelbegin \DIFdel{we still do not possess}\DIFdelend \DIFaddbegin \DIFadd{is difficult to determine}\DIFaddend , so that using \DIFdelbegin \DIFdel{default
uninformative priors }\DIFdelend \DIFaddbegin \DIFadd{the
default priors provided by brms }\DIFaddend is, for the \DIFdelbegin \DIFdel{time being, theoretically sound}\DIFdelend \DIFaddbegin \DIFadd{purposes of this article
(and initial analyses), an acceptable practice}\DIFaddend . For practical guidance
on prior specification \DIFdelbegin \DIFdel{, see
}%DIFDELCMD < \url{https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations}%%%
\DIFdelend \DIFaddbegin \DIFadd{and prior predictive checks, see
\mbox{%DIFAUXCMD
\citet{bockting2024}}\hskip0pt%DIFAUXCMD
, \mbox{%DIFAUXCMD
\citet{mikkola2024}}\hskip0pt%DIFAUXCMD
,
\mbox{%DIFAUXCMD
\citet{standevelopmentteam2025} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet[p.~82-84]{mcelreath2020}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
quantify (un)certainty. \DIFaddbegin \DIFadd{When obtaining the summary of the posterior
probability distributions, the mean and standard deviation of the
posterior distribution, together with }\DIFaddend Bayesian Credible Intervals \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{are
given for each parameter. The mean and standard deviation are commonly
used metrics, given that posteriors tend to be approximately Gaussian.
An alternative are the median and the median absolute deviation, which
can be obtained by setting }\texttt{\DIFadd{robust\ =\ TRUE}} \DIFadd{in
}\texttt{\DIFadd{summary()}} \DIFadd{(note that the nature of the posterior distribution
of the parameters is not related to the nature of the distribution
family chosen for the outcome, so while the outcome is modelled with a
beta distribution, the posteriors of the parameters follow a symmetric
distribution due to the central limit theorem). Bayesian Credible
Intervals (}\DIFaddend CrIs) can be calculated from the posterior distributions at
several probability levels (e.g.~95, 90, 80, 60\%) for a more complete
view on estimated parameters. Bayesian CrIs indicate that, at a certain
probability level, the parameter's estimate lies within that interval:
so, for example, a 90\% CrI {[}A, B{]} indicates that there is a 90\%
probability that the estimate is between A and B. Different probability
levels correspond to different levels of confidence: the higher the
probability the higher the confidence (always conditional on data and
model). Readers interested in a full and accessible exposition of
Bayesian statistics are referred to \citet{mcelreath2019}. Shorter
introductions can be found in \citet{etz2018}, \citet{vasishth2018} and
\citet{nalborczyk2019}.

\DIFaddbegin \DIFadd{Beta regression models return coefficient estimates in log-odds,
similarly to Bernoulli/binomial (aka logistic) regression models.
Log-odds correspond to the logarithm of the odds
\(\log\left(\frac{p}{1 - p}\right)\). To convert log-odds into
probabilities (or into a variable in the }{[}\DIFadd{0, 1}{]} \DIFadd{interval), the
inverse logit function is used \(\frac{1}{1 + e^{-x}}\), where \(x\) is
the log-odds value. Figure~\ref{fig-p-log-odds} illustrates the
correspondence between log-odds and the }{[}\DIFadd{0, 1}{]} \DIFadd{values. In R, the
inverse logit can be obtained with the base R function
}\texttt{\DIFadd{plogis()}}\DIFadd{, while the logit can be obtained with the base R
function }\texttt{\DIFadd{qlogis()}}\DIFadd{. Another relevant transformation of log-odds
is exponentiation (with base R }\texttt{\DIFadd{exp()}}\DIFadd{) which returns the
corresponding odds-ratio: odds-ratios indicate the rate of change in the
values of the outcome variable relative to a baseline. An odds-ratio of
1 indicates no change, positive odds-ratios indicate an increase, while
negative odds-ratios correspond to a decrease. For example, if the
odds-ratio of the duration of /a/ vs the duration of /i/ is 1.2, this
means that the duration of /a/ is grater than that of /i/. Moreover,
odds-ratios can be interpreted as percentage change: an odds-ratio of
1.2 corresponds to a 20\% }\emph{\DIFadd{increase}} \DIFadd{relative to the baseline, an
odds-ratio of 0.8 corresponds to a 20\% }\emph{\DIFadd{decrease}} \DIFadd{relative to the
baseline, while an odds-ratio of 1 means a 0\% change (i.e.~no change).
}
...
\pandocbounded{\includegraphics[keepaspectratio]{manuscript_files/figure-pdf/fig-p-log-odds-1.pdf}}

}

\caption{\label{fig-p-log-odds}\DIFaddFL{Correspondence between log-odds and
probabilities.}}

\end{figure}%DIF > 

\DIFaddend \section{Case study 1: coarticulatory voicing within consonant
...
different voicing proportions, but for sake of space we restrict this
...
for centred speech rate.\footnote{While the terms ``random effects,
  ...
  and hierarchical terms. \DIFaddbegin \DIFadd{Note that with ``varying terms'' we refer
  specifically to model terms with partial pooling in opposition to
  ``constant terms'' estimated with no pooling
  \mbox{%DIFAUXCMD
\citep[p.~408]{mcelreath2020}}\hskip0pt%DIFAUXCMD
. See \mbox{%DIFAUXCMD
\citet{nalborczyk2019} }\hskip0pt%DIFAUXCMD
for a
  detailed overview.}\DIFaddend } The categorical predictors vowel and C2 are coded
using indexing rather than the traditional R contrasts \DIFaddbegin \DIFadd{(such as
treatment or sum)}\DIFaddend , as advocated in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{mcelreath2019}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet[p 155, Ch 8]{mcelreath2019}}\hskip0pt%DIFAUXCMD
}\DIFaddend :
in R, this corresponds to suppressing the model's intercept with the
\texttt{0\ +} syntax\DIFdelbegin \DIFdel{; using indexing instead of contrasts makes it easier to specify priors and
uncertainty
is distributed equally }\DIFdelend \DIFaddbegin \DIFadd{. One benefit of using indexing is that uncertainty
is equally distributed }\DIFaddend across all levels of categorical predictors\DIFdelbegin \DIFdel{. For pedagogical simplicity, the model will use the default
priors as }\DIFdelend \DIFaddbegin \DIFadd{,
since priors apply on the mean of each level, rather than on the
difference of subsequent levels from the reference level. The latter
scenario, which applies in traditional contrasts, including treatment
and sum, makes estimates of the last level in the predictor more
uncertain than the estimates of the other levels, given that the
uncertainty of the intercept is added to the uncertainty of the
difference. As }\DIFaddend explained in Section~\ref{sec-bayes}\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, using the default
priors calculated by brms is an acceptable method in the absence of very
specific quantitative expectation, given that the default priors are
weak enough to bear little effect on posteriors. Researchers should use
their own judgement and expertise in a case by case fashion.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{(i.e.~the mean of the posterior distribution calculated from the MCMC
draws)}\DIFaddend , the estimate error \DIFaddbegin \DIFadd{(i.e.~the standard deviation of the posterior
distribution) }\DIFaddend and the lower and upper limits of a Bayesian Credible
interval (CrI) \DIFaddbegin \DIFadd{of the posterior distribution}\DIFaddend . Note that all values are
in log-odds, since the link function in beta regressions is the logistic
function, like in binomial/Bernoulli regressions. Here, we print an 80\%
CrI. There is nothing special about 95\% CrI within Bayesian inference
and instead experts recommend to check and report a variety of CrIs.
Obtaining CrIs at different probability levels allows researchers to
make more fine-grained inferential statements than the frequentist
...
Log-odds can be converted to \DIFdelbegin \DIFdel{odd-ratios }\DIFdelend \DIFaddbegin \DIFadd{odds-ratios }\DIFaddend by exponentiating the value:
0.01-0.15 log odds correspond to an \DIFdelbegin \DIFdel{odd-ratio }\DIFdelend \DIFaddbegin \DIFadd{odds-ratio }\DIFaddend of 1.01 to 1.16, or as
...
\DIFaddbegin \DIFadd{posteriors of the coefficients for vowel and C2 are the estimated
\(\mu\)s of the beta distribution, since the beta distribution is
parametrised with \(\mu\) and \(\phi\). We will not discuss \(\phi\),
although the estimate of this parameter can be found in the summary of
the model, under ``Further Distributional Parameters''.The }\DIFaddend CrIs of the
vowel/C2 coefficients span all negative log-odds values: these
correspond to proportions that are lower than 0.5 (which is 0 in
...
%DIFDELCMD < Warning: Dropping 'draws_df' class as required metadata was removed.
%DIFDELCMD < \end{verbatim}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{longtable}[]{@{}
...
\NormalTok{voi\_prop\_bm\_draws\_long }\SpecialCharTok{|\textgreater{}} 
  \DIFaddbegin \DIFadd{\FunctionTok{mutate}\NormalTok{(}
    \AttributeTok{c2 =} \FunctionTok{str\_remove}\NormalTok{(c2, }\StringTok{"c2"}\NormalTok{),}
    \AttributeTok{vowel =} \FunctionTok{str\_remove}\NormalTok{(vowel, }\StringTok{"vowel"}\NormalTok{)}
\NormalTok{  ) }\SpecialCharTok{|\textgreater{}} 
  }\DIFaddend \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\FunctionTok{plogis}\NormalTok{(value), vowel)) }\SpecialCharTok{+}
  ...
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{caption =} \StringTok{"The points are the medians and the two lines are 60 and 80\% CrIs."}\DIFaddbegin \DIFadd{\NormalTok{,}
    \AttributeTok{x =} \StringTok{"Voicing during closure (prop)"}\NormalTok{,}
    \AttributeTok{y =} \StringTok{"Vowel"}
}\DIFaddend \NormalTok{  )}
\end{Highlighting}
\end{Shaded}
...
three consonants /k, t, p/? \DIFdelbegin \DIFdel{One approach is to }\DIFdelend \DIFaddbegin \DIFadd{We can ``marginalise'' over vowel, i.e.~}\DIFaddend take
the mean across vowels within each consonant for each posterior draw,
and the posterior distribution of the resulting list of values is the
predicted posterior distribution of voicing proportion for each
consonant, assuming an ``average'' vowel. See
Figure~\ref{fig-voi-pbdl-c2} \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{for the density of these posteriors. For a
detailed treatment of various approaches to marginalisation that apply
to more complex cases like those involving numeric predictors, see
\mbox{%DIFAUXCMD
\citet{arelbundock2024}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
\NormalTok{voi\_prop\_bm\_draws\_long\_c2 }\SpecialCharTok{|\textgreater{}} 
  \DIFaddbegin \DIFadd{\FunctionTok{mutate}\NormalTok{(}
    \AttributeTok{c2 =} \FunctionTok{str\_remove}\NormalTok{(c2, }\StringTok{"c2"}\NormalTok{)}
\NormalTok{  ) }\SpecialCharTok{|\textgreater{}} 
  }\DIFaddend \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\FunctionTok{plogis}\NormalTok{(value\_mean), c2)) }\SpecialCharTok{+}
  \FunctionTok{stat\_halfeye}\NormalTok{(}\AttributeTok{.width =} \FunctionTok{c}\NormalTok{(}\FloatTok{0.6}\NormalTok{, }\FloatTok{0.8}\NormalTok{)) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{caption =} \StringTok{"The points are the medians and the two lines are 60 and 80\% CrIs."}\DIFaddbegin \DIFadd{\NormalTok{,}
    \AttributeTok{x =} \StringTok{"Voicing during closure (prop)"}\NormalTok{,}
    \AttributeTok{y =} \StringTok{"C2"}
}\DIFaddend \NormalTok{  )}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{voi\_prop\_bm\_diff }\SpecialCharTok{|\textgreater{}} 
  ...
  \FunctionTok{geom\_vline}\NormalTok{(}\AttributeTok{xintercept =} \DecValTok{0}\DIFdelbegin \DIFdel{\NormalTok{)}
}\DIFdelend \DIFaddbegin \DIFadd{\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{x =} \StringTok{"Log{-}odds difference in /p/ vs /t/"}\NormalTok{,}
...
log-odds! We can convert these into \DIFdelbegin \DIFdel{odd-ratios }\DIFdelend \DIFaddbegin \DIFadd{odds-ratios }\DIFaddend with \texttt{exp()}. The
output of the code below is shown in Table~\ref{tbl-voi-bm-quant}.
\DIFdelbegin \DIFdel{Odd-ratios }\DIFdelend \DIFaddbegin \DIFadd{Odds-ratios }\DIFaddend indicate the ratio of the difference between A and B, so
that 1 means no difference, values greater than 1 indicate an increase
in A relative to B and values lower than 1 indicate a decrease in A
relative to B. \DIFdelbegin \DIFdel{Odd-ratios }\DIFdelend \DIFaddbegin \DIFadd{Odds-ratios }\DIFaddend are useful when looking at differences that
are in log-odds because while the relative magnitude of the difference
in proportion between two groups is the same independent of the baseline
proportion, the \emph{absolute} magnitude of the difference depends on
the baseline value. For example, an \DIFdelbegin \DIFdel{odd-ratio }\DIFdelend \DIFaddbegin \DIFadd{odds-ratio }\DIFaddend difference of 1.25 would
...
tutorial we just focus on \DIFdelbegin \DIFdel{odd-ratios }\DIFdelend \DIFaddbegin \DIFadd{odds-ratios }\DIFaddend for simplicity.
...
\begin{Shaded}
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\NormalTok{ce }\OtherTok{\textless{}{-}} }\DIFaddend \FunctionTok{conditional\_effects}\NormalTok{(voi\_prop\_bm, }\StringTok{"c2:vowel"}\NormalTok{)}
\DIFaddbegin \DIFadd{\NormalTok{p }\OtherTok{\textless{}{-}} \FunctionTok{plot}\NormalTok{(ce, }\AttributeTok{plot =} \ConstantTok{FALSE}\NormalTok{)[[}\DecValTok{1}\NormalTok{]]}
\NormalTok{p }\SpecialCharTok{+} \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"C2"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Voicing during closure (prop)"}\NormalTok{)}
}\DIFaddend \end{Highlighting}
\end{Shaded}
...
of the two functions is shown in Table~\ref{tbl-marginal-1} and
Table~\ref{tbl-marginal-2}.\DIFaddbegin \footnote{\DIFadd{For a discussion on multiple
  comparisons in Bayesian statistics, see \mbox{%DIFAUXCMD
\citet{gelman2012}}\hskip0pt%DIFAUXCMD
. If
  multiple comparisons fixed effects is performed and one worries about
  their joint frequentist error rates, one could, for example, increase
  the width of the credible intervals for more conservative inference.}}
\DIFaddend 
...
\bottomrule\noalign{}
\endlastfoot
c2 & \DIFdelbegin \DIFdel{mean(t) }\DIFdelend \DIFaddbegin \DIFadd{p }\DIFaddend - \DIFdelbegin \DIFdel{mean(k) }\DIFdelend \DIFaddbegin \DIFadd{k }\DIFaddend & \DIFdelbegin \DIFdel{0.035 }\DIFdelend \DIFaddbegin \DIFadd{0.218 }\DIFaddend & \DIFdelbegin \DIFdel{-0.004 }\DIFdelend \DIFaddbegin \DIFadd{0.178 }\DIFaddend & \DIFdelbegin \DIFdel{0.074 }\DIFdelend \DIFaddbegin \DIFadd{0.256 }\DIFaddend \\
c2 & \DIFdelbegin \DIFdel{mean(p ) }\DIFdelend \DIFaddbegin \DIFadd{p }\DIFaddend - \DIFdelbegin \DIFdel{mean(k ) }\DIFdelend \DIFaddbegin \DIFadd{t }\DIFaddend & \DIFdelbegin \DIFdel{0.218 }\DIFdelend \DIFaddbegin \DIFadd{0.183 }\DIFaddend & \DIFdelbegin \DIFdel{0.178 }\DIFdelend \DIFaddbegin \DIFadd{0.145 }\DIFaddend & \DIFdelbegin \DIFdel{0.256 }\DIFdelend \DIFaddbegin \DIFadd{0.219 }\DIFaddend \\
c2 & \DIFdelbegin \DIFdel{mean(p ) }\DIFdelend \DIFaddbegin \DIFadd{t }\DIFaddend - \DIFdelbegin \DIFdel{mean(t ) }\DIFdelend \DIFaddbegin \DIFadd{k }\DIFaddend & \DIFdelbegin \DIFdel{0.183 }\DIFdelend \DIFaddbegin \DIFadd{0.035 }\DIFaddend & \DIFdelbegin \DIFdel{0.145 }\DIFdelend \DIFaddbegin \DIFadd{-0.004 }\DIFaddend & \DIFdelbegin \DIFdel{0.219 }\DIFdelend \DIFaddbegin \DIFadd{0.074 }\DIFaddend \\
...
\begin{Shaded}
\begin{Highlighting}[]
\DIFaddbegin \DIFadd{\NormalTok{ce }\OtherTok{\textless{}{-}} }\DIFaddend \FunctionTok{conditional\_effects}\NormalTok{(nas\_prop\_bm, }\StringTok{"vowel:voicing"}\NormalTok{)}
\DIFaddbegin \DIFadd{\NormalTok{p }\OtherTok{\textless{}{-}} \FunctionTok{plot}\NormalTok{(ce, }\AttributeTok{plot =} \ConstantTok{FALSE}\NormalTok{)[[}\DecValTok{1}\NormalTok{]]}
\NormalTok{p }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{y =} \StringTok{"Proportion of nasality"}
\NormalTok{  )}
}\DIFaddend \end{Highlighting}
\end{Shaded}
...
following code is \DIFdelbegin \DIFdel{show }\DIFdelend \DIFaddbegin \DIFadd{shown }\DIFaddend in Table~\ref{tbl-nas-long-voi} and the density
...
\NormalTok{nas\_prop\_bm\_draws\_long\_voicing }\SpecialCharTok{|\textgreater{}} 
  \DIFaddbegin \DIFadd{\FunctionTok{mutate}\NormalTok{(}
    \AttributeTok{voicing =} \FunctionTok{str\_remove}\NormalTok{(voicing, }\StringTok{"voicing"}\NormalTok{)}
\NormalTok{  ) }\SpecialCharTok{|\textgreater{}} 
  }\DIFaddend \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(value\_mean\_prop, voicing)) }\SpecialCharTok{+}
  \FunctionTok{stat\_halfeye}\NormalTok{(}\AttributeTok{.width =} \FunctionTok{c}\NormalTok{(}\FloatTok{0.6}\NormalTok{, }\FloatTok{0.8}\DIFdelbegin \DIFdel{\NormalTok{))}
}\DIFdelend \DIFaddbegin \DIFadd{\NormalTok{)) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{x =} \StringTok{"Nasalisation proportion"}
\NormalTok{  )}
}\DIFaddend \end{Highlighting}
\end{Shaded}
...
voiceless NC sequences. Let's quantify how \DIFdelbegin \DIFdel{greater }\DIFdelend \DIFaddbegin \DIFadd{much greater it is, }\DIFaddend as we did
in Section~\ref{sec-case-1}. We will use \DIFdelbegin \DIFdel{odd-ratios }\DIFdelend \DIFaddbegin \DIFadd{odds-ratios }\DIFaddend in this context as
well, i.e.~we will convert log-odds to \DIFdelbegin \DIFdel{odd-ratios }\DIFdelend \DIFaddbegin \DIFadd{odds-ratios }\DIFaddend using the
...
\NormalTok{nas\_prop\_r }\SpecialCharTok{|\textgreater{}} 
  ...
  \FunctionTok{lims}\NormalTok{(}\AttributeTok{x =} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{, }\FloatTok{1.5}\DIFdelbegin \DIFdel{\NormalTok{))}
}\DIFdelend \DIFaddbegin \DIFadd{\NormalTok{)) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{x =} \StringTok{"Log{-}odds difference between voiceless vs voiced NC sequences"}\NormalTok{,}
    \AttributeTok{y =} \StringTok{"Speaker"}
\NormalTok{  )}
}\DIFaddend \end{Highlighting}
\end{Shaded}
...
\DIFdelbegin %DIFDELCMD < \bibliography{linguistics.bib,statistics.bib}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{references.bib}
\DIFaddend 
