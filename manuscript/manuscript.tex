...
Another commonly employed type of outcome variable is data that is
bounded between 0 and 1: for example, proportion of voicing during
closure \citep{davidson2016}, vocal folds contact quotient
\citep{herbst2017}, gesture amplitude \citep{carignan2021}, nasalance
\citep{carignan2021}, and data from slider answers where participants
can indicate responses for qualities.
...
Furthermore, a Gaussian distribution predicts values above 1, which are
not possible given the bounded nature of the data. Moreover, the density
...
distribution for this purpose is the beta distribution. Note that the
beta distribution supports variables in the interval (0, 1),
i.e.~excluding 0 and 1; variables in the interval {[}0, 1{]}, including
0 and 1, can be modelled with zero and zero/one inflated beta
distributions (see Section~\ref{sec-beta}). This tutorial introduces
...
the modelling of unit interval data, i.e.~data that are in the interval
{[}0, 1{]} \citep{ferrari_beta_2004, cribari-neto_beta_2010}. It has
...
variance
(\(\text{Var}(y) = \frac{\mu \, (1 - \mu)}{\phi + 1}\)).\footnote{The
  ...
  location parameter.} That is, the larger \(\phi\) the smaller the
...
See Figure~\ref{fig-beta} for an illustration of different beta
...
\caption{\label{fig-beta}Beta distributions for various \(\mu\),
\(\phi\) values.}

\end{figure}%

\textcolor[HTML]{b22222}{As mentioned earlier, the beta distribution
handles data in the interval (0, 1), thus excluding 0 and 1. To model
{[}0, 1{]} data (including 0 and 1), the zero and zero/one inflated
extensions of the beta distribution can be used. Both of these are
supported in the software used in this tutorial, but will not be treated
here. We refer readers to \citet{heiss2021}. A simpler alternative is to
transform the 0s and 1s in the data by adding/subtracting a very small
value to/from them, like 0.001. The transformation ensures all values in
the data are between 0 and 1, but not exactly 0 and 1.}
...
some areas of phonetics is difficult to determine, so that using the
default priors provided by brms is, for the purposes of this article
(and initial analyses), an acceptable practice. For practical guidance
on prior specification and prior predictive checks, see
\citet{bockting2024}, \citet{mikkola2024},
\citet{standevelopmentteam2025} and \citet[p.~82-84]{mcelreath2020}.
...
quantify (un)certainty. When obtaining the summary of the posterior
probability distributions, the mean and standard deviation of the
posterior distribution, together with Bayesian Credible Intervals are
...
can be obtained by setting \texttt{robust\ =\ TRUE} in
\texttt{summary()} (note that the nature of the posterior distribution
...
Intervals (CrIs) can be calculated from the posterior distributions at
...
Beta regression models return coefficient estimates in log odds,
similarly to Bernoulli/binomial (aka logistic) regression models. Log
odds correspond to the logarithm of the odds
\(\log\left(\frac{p}{1 - p}\right)\). To convert log odds into
probabilities (or into a variable in the {[}0, 1{]} interval), the
inverse logit function is used \(\frac{1}{1 + e^{-x}}\), where \(x\) is
the log-odds value. Figure~\ref{fig-p-log-odds} illustrates the
correspondence between log odds and the {[}0, 1{]} values. In R, the
inverse logit can be obtained with the base R function
\texttt{plogis()}, while the logit can be obtained with the base R
function \texttt{qlogis()}. Another relevant transformation of log odds
is exponentiation (with base R \texttt{exp()}) which returns the
...
negative odds ratios correspond to a decrease. For example, if the odds
ratio of the duration of /a/ vs the duration of /i/ is 1.2, this means
that the duration of /a/ is grater than that of /i/. Moreover, odds
ratios can be interpreted as percentage change: an odds ratio of 1.2
corresponds to a 20\% \emph{increase} relative to the baseline, an odds
ratio of 0.8 corresponds to a 20\% \emph{decrease} relative to the
...
\caption{\label{fig-p-log-odds}Correspondence between log odds and
probabilities.}
...
for centred speech rate.\footnote{While the terms ``random effects,
  ...
  and hierarchical terms. Note that with ``varying terms'' we refer
  specifically to model terms with partial pooling in opposition to
  ``constant terms'' estimated with no pooling
  \citep[p.~408]{mcelreath2020}. See \citet{nalborczyk2019} for a
  detailed overview.} The categorical predictors vowel and C2 are coded
using indexing rather than the traditional R contrasts (such as
treatment or sum), as advocated in \citet[p 155, Ch 8]{mcelreath2019}:
in R, this corresponds to suppressing the model's intercept with the
\texttt{0\ +} syntax. \textcolor[HTML]{b22222}{With this approach, the
model's coefficients do not represent differences from a reference level
(normally the intercept), rather they represent the predicted mean for
all combinations of each categorical predictor's levels (no contrasts
are applied). The following code shows the coding matrix for a
hypothetical predictor with three levels, coded using treatment
contrasts and indexing.}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{fac }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"a"}\NormalTok{, }\StringTok{"b"}\NormalTok{, }\StringTok{"c"}\NormalTok{))}

\CommentTok{\# Treatment contrasts}
\FunctionTok{contrasts}\NormalTok{(fac)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\# Indexing}
\FunctionTok{diag}\NormalTok{(}\FunctionTok{nlevels}\NormalTok{(fac))}
\end{Highlighting}
\end{Shaded}
...
\textcolor[HTML]{b22222}{With indexing, a model would have one
coefficient for the mean of ``a'', one for the mean of ``b'', and one
for the mean of ``c''. No comparisons between levels are present in the
matrix, but these can be obtained from the model post-hoc (as we do
below).}

One benefit of using indexing is that uncertainty is equally distributed
across all levels of categorical predictors, since priors apply on the
mean of each level, rather than on the difference of subsequent levels
from the reference level. The latter scenario, which applies in
traditional contrasts, including treatment and sum, makes estimates of
the last level in the predictor more uncertain than the estimates of the
other levels, given that the uncertainty of the intercept is added to
the uncertainty of the difference. As explained in
Section~\ref{sec-bayes}, using the default priors calculated by brms is
an acceptable method in the absence of very specific quantitative
expectation, given that the default priors are weak enough to bear
little effect on posteriors. Researchers should use their own judgement
and expertise in a case by case fashion.
...
(i.e.~the mean of the posterior distribution calculated from the MCMC
draws), the estimate error (i.e.~the standard deviation of the posterior
distribution) and the lower and upper limits of a Bayesian Credible
interval (CrI) of the posterior distribution. Note that all values are
...
between 0.01 and 0.15 log odds {[}\(\beta\) = 0.08, SD = 0.06{]}. Log
odds can be converted to odds ratios by exponentiating the value:
0.01-0.15 log odds correspond to an odds ratio of 1.01 to 1.16, or as
...
posteriors of the coefficients for vowel and C2 are the estimated
...
the model, under ``Further Distributional Parameters''.The CrIs of the
vowel/C2 coefficients span all negative log-odds values: these
correspond to proportions that are lower than 0.5 (which is 0 in log
odds). This matches the general trends in the raw data, which we plotted
in Figure~\ref{fig-ita-egg}. Next, we will plot the predicted
...
\NormalTok{voi\_prop\_bm\_draws\_long }\SpecialCharTok{|\textgreater{}} 
  \FunctionTok{mutate}\NormalTok{(}
    \AttributeTok{c2 =} \FunctionTok{str\_remove}\NormalTok{(c2, }\StringTok{"c2"}\NormalTok{) }\SpecialCharTok{|\textgreater{}} \FunctionTok{factor}\NormalTok{(}\AttributeTok{levels =} \FunctionTok{c}\NormalTok{(}\StringTok{"k"}\NormalTok{, }\StringTok{"t"}\NormalTok{, }\StringTok{"p"}\NormalTok{)),}
...
\NormalTok{  ) }\SpecialCharTok{|\textgreater{}} 
  ...
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{caption =} \StringTok{"The points are the medians and the two lines are 60 and 80\% CrIs."}\NormalTok{,}
...
three consonants /k, t, p/? We can ``marginalise'' over vowel, i.e.~take
...
Figure~\ref{fig-voi-pbdl-c2} for the density of these posteriors. For a
...
/k, t/ (all averaged across vowels). Since we want to compare /k, t/
with /p/, we should first average the average draws of /k, t/ and then
take the difference of the averaged /k, t/ draws and the draws of /p/.
...
\NormalTok{voi\_prop\_bm\_diff }\OtherTok{\textless{}{-}}\NormalTok{ voi\_prop\_bm\_draws\_long\_c2 }\SpecialCharTok{|\textgreater{}} 
  ...
  \FunctionTok{mutate}\NormalTok{(}
    ...
    \CommentTok{\# calculate the difference of /p/ and /k, t/}
...
voicing proportion between /k, t/ and /p/.}
...
\NormalTok{voi\_prop\_bm\_diff }\SpecialCharTok{|\textgreater{}} 
  ...
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{x =} \StringTok{"Log{-}odds difference in /p/ vs /k, t/"}\NormalTok{,}
...
between the expected voicing proportion of /k, t/ vs /p/, averaged
across vowels.}
...
\citep{burkner2024}. Beware that the values of the difference are in log
odds! We can convert these into odds ratios with \texttt{exp()}. The
output of the code below is shown in Table~\ref{tbl-voi-bm-quant}. Odds
ratios indicate the ratio of the difference between A and B, so that 1
means no difference, values greater than 1 indicate an increase in A
relative to B and values lower than 1 indicate a decrease in A relative
to B. Odds ratios are useful when looking at differences that are in log
odds because while the relative magnitude of the difference in
...
the baseline value. For example, an odds ratio difference of 1.25 would
...
tutorial we just focus on odds ratios for simplicity.
...
than in /k, t/. At 80\% confidence, the change ratio is 1.10-1.34 (or
...
proportion in /p/ is greater than in /k, t/ and that the increase is
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ce }\OtherTok{\textless{}{-}} \FunctionTok{conditional\_effects}\NormalTok{(voi\_prop\_bm, }\StringTok{"c2:vowel"}\NormalTok{)}
\NormalTok{p }\OtherTok{\textless{}{-}} \FunctionTok{plot}\NormalTok{(ce, }\AttributeTok{plot =} \ConstantTok{FALSE}\NormalTok{)[[}\DecValTok{1}\NormalTok{]]}
...
Table~\ref{tbl-marginal-2}.\footnote{For a discussion on multiple
  comparisons in Bayesian statistics, see \citet{gelman2012}. If
...
\bottomrule\noalign{}
\endlastfoot
c2 & p - k & 0.218 & 0.178 & 0.256 \\
c2 & p - t & 0.183 & 0.145 & 0.219 \\
c2 & t - k & 0.035 & -0.004 & 0.074 \\
...
\textcolor[HTML]{b22222}{In sum, we used a Bayesian beta regression
model to assess whether different places of articulation affect the
proportion of coarticulatory progressive voicing. The expectation was
that that the proportion of progressive voicing should increase with
fronter places of articulation (so from /k/ to /t/ to /p/). This
expectation is not borne out by the results: while the proportion of
voicing is robustly greater in /p/ than in /k, t/ (at 90\% probability,
the voicing proportion in /p/ is 7-38\% higher), the difference in
voicing proportion between /k/ and /t/ is very small and uncertain.}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ce }\OtherTok{\textless{}{-}} \FunctionTok{conditional\_effects}\NormalTok{(nas\_prop\_bm, }\StringTok{"vowel:voicing"}\NormalTok{)}
\NormalTok{p }\OtherTok{\textless{}{-}} \FunctionTok{plot}\NormalTok{(ce, }\AttributeTok{plot =} \ConstantTok{FALSE}\NormalTok{)[[}\DecValTok{1}\NormalTok{]]}
...
following code is shown in Table~\ref{tbl-nas-long-voi} and the density
...
voiceless NC sequences. Let's quantify how much greater it is, as we did
in Section~\ref{sec-case-1}. We will use odds ratios in this context as
well, i.e.~we will convert log odds to odds ratios using the
...
\NormalTok{nas\_prop\_r }\SpecialCharTok{|\textgreater{}} 
  ...
  \FunctionTok{lims}\NormalTok{(}\AttributeTok{x =} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{, }\FloatTok{1.5}\NormalTok{)) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
...
\textcolor[HTML]{b22222}{To conclude, previous work on coarticulatory
nasalisation in English has suggested that vowels followed by an NC
sequence where C is voiceless (NT) should show earlier coarticulatory
nasalisation than vowels followed by an NC sequence where C is voiced
(ND). We fitted a Bayesian beta regression model to assess this
expectation. In sum, we find an increase of nasalisation proportion in
the voiceless NC sequences compared to that of voiced NC sequences, with
a 90\% probability that the increase is between 23\% and 69\% of the
proportion of voiced NC sequences. As for individual speakers'
variability, we find that while most speakers are more likely to have a
larger nasalisation proportion in voiceless NC sequences, a few might
