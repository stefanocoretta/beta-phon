...
As mentioned earlier, the beta distribution handles data in the interval
(0, 1), thus excluding 0 and 1. To model {[}0, 1{]} data (including 0
...
\texttt{0\ +} syntax.
\textcolor{red}{With this approach, the model’s coefficients do not represent differences from a reference level (normally the intercept), rather they represent the predicted mean for all combinations of each categorical predictor’s levels (no contrasts are applied). The following code shows the coding matrix for a hypothetical predictors with three levels, coded using treatment contrasts and indexing.}
...
\textcolor{red}{With indexing, a model would have one coefficient for the mean of “a”, one for the mean of “b”, and one for the mean of “c”. No comparisons between levels are present in the matrix, but these can be obtained from the model post-hoc (as we do below).}
