...
1.2 corresponds to a 20\% \emph{increase} relative to the baseline, an
odds-ratio of 0.8 corresponds to a 20\% \emph{decrease} relative to the
...
\texttt{0\ +} syntax. One benefit of using indexing is that uncertainty
is equally distributed across all levels of categorical predictors,
...
difference. As explained in Section~\ref{sec-bayes}, using the default
...
Log-odds can be converted to odds-ratios by exponentiating the value:
...
log-odds! We can convert these into odds-ratios with \texttt{exp()}. The
output of the code below is shown in Table~\ref{tbl-voi-bm-quant}.
Odds-ratios indicate the ratio of the difference between A and B, so
...
relative to B. Odds-ratios are useful when looking at differences that
