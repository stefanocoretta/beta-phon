...
}\DIFdelend \DIFaddbegin \DIFadd{, and data from slider answers where participants
can indicate responses of qualities from 0 to 100.
}\DIFaddend 
...
the modelling of unit interval data\DIFaddbegin \DIFadd{, i.e.~data that is in the interval
...
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{and prior predictive checks, see
\mbox{%DIFAUXCMD
\citet{bockting2024}}\hskip0pt%DIFAUXCMD
, \mbox{%DIFAUXCMD
\citet{mikkola2024} }\hskip0pt%DIFAUXCMD
and the Wiki page
https://github.com/stan-dev/stan/wiki/Prior-Choice, and
\mbox{%DIFAUXCMD
...
given for each parameter. The mean and standard deviation are
appropriate since, due to the central limit theorem, the draws from the
MCMC chains for each parameter usually follow a symmetric distribution
(note that the nature of the posterior distribution of the parameters is
not related to the nature of the distribution family chosen for the
outcome, so while the outcome is modelled with a beta distribution, the
posteriors of the parameters follow a symmetric distribution due to the
central limit theorem). Bayesian Credible Intervals (}\DIFaddend CrIs) can be
...
is equally distributed }\DIFaddend across all levels of categorical predictors\DIFdelbegin \DIFdel{. For pedagogical simplicity,
...
Table~\ref{tbl-marginal-2}. \DIFaddbegin \DIFadd{Note that even when multiple comparisons are
made, there is no need to apply any correction as it is commonly done
with }\emph{\DIFadd{p}}\DIFadd{-values in frequentist statistics
\mbox{%DIFAUXCMD
\citep[see][]{gelman2012}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
